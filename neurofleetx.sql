...
CREATE TABLE IF NOT EXISTS vehicles (
    ...
    current_fuel_level DECIMAL(5,2) NOT NULL DEFAULT 100.00,
...
-- Create bookings table (no CHECK constraint to maximize compatibility)
...
-- Add foreign keys after tables are created to avoid ordering issues
...
ALTER TABLE bookings
    ...
    ADD CONSTRAINT fk_booking_vehicle FOREIGN KEY (vehicle_id) REFERENCES vehicles(id) ON DELETE CASCADE,
    ADD CONSTRAINT fk_booking_driver FOREIGN KEY (driver_id) REFERENCES users(id) ON DELETE SET NULL;

-- Insert sample users (replace hashed passwords before production)
...
-- Assign drivers to vehicles using JOIN updates (safe)
UPDATE vehicles v JOIN users u ON u.email = 'driver1@neurofleetx.com' SET v.driver_id = u.id WHERE v.license_plate = 'ABC-123';
UPDATE vehicles v JOIN users u ON u.email = 'driver2@neurofleetx.com' SET v.driver_id = u.id WHERE v.license_plate = 'XYZ-789';
UPDATE vehicles v JOIN users u ON u.email = 'driver1@neurofleetx.com' SET v.driver_id = u.id WHERE v.license_plate = 'EV-001';
...
((SELECT id FROM users WHERE email = 'customer1@neurofleetx.com'),
 ...
 '123 Main St, New York, NY', '456 Park Ave, New York, NY', DATE_ADD(NOW(), INTERVAL 2 HOUR), 25.50, 2, 'CONFIRMED', 'PAID', NOW(), NOW()),
((SELECT id FROM users WHERE email = 'customer2@neurofleetx.com'),
 (SELECT id FROM vehicles WHERE license_plate = 'EV-001'),
 '789 Broadway, New York, NY', '321 5th Ave, New York, NY', DATE_ADD(NOW(), INTERVAL 4 HOUR), 35.75, 1, 'PENDING', 'PENDING', NOW(), NOW()),
((SELECT id FROM users WHERE email = 'customer1@neurofleetx.com'),
 (SELECT id FROM vehicles WHERE license_plate = 'XYZ-789'),
 'Airport Terminal 4, Queens, NY', 'Times Square, New York, NY', DATE_ADD(NOW(), INTERVAL 1 DAY), 85.00, 3, 'COMPLETED', 'PAID', DATE_SUB(NOW(), INTERVAL 1 DAY), DATE_SUB(NOW(), INTERVAL 1 DAY));
